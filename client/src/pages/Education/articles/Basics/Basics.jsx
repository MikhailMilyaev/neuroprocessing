...
const sortChildrenByNumber = (nodes) => {
  ...
  const walk = (arr) =>
    arr.map((n) => {
      const ch = n.children?.length ? [...n.children].sort((a,b)=>{
  ...
  const Section = ({ node }) => {
    ...
    return (
      <section className={styles.section} aria-labelledby={node.id}>
        <Tag id={node.id} data-anchorable="1" className={node.level >= 2 ? styles.subsectionTitle : undefined}>
          {node.title}
        </Tag>

        <div className={styles.sectionBody} dangerouslySetInnerHTML={{ __html: node.content_html }} />
        ...
        {node.refs_html ? (
          <div className={styles.refsBlock} aria-label={`Источники к разделу ${node.title}`}>
            ...
            <div className={styles.refsList} dangerouslySetInnerHTML={{ __html: node.refs_html }} />
          </div>
        ) : null}

        {node.children?.length ? node.children.map((ch) => <Section key={ch.id} node={ch} />) : null}
      </section>
    );
  };
    ...
    <main className={styles.page}>
          ...
          <nav>
            {tocItems.map((item) => (
              <div key={item.id} className={tocIndent(item.level)}>
                <a className={`${styles.tocLink} ${activeId === item.id ? styles.tocLinkActive : ""}`} href={`#${item.id}`}>
                  {item.title}
                </a>
              </div>
            ))}
      ...
      <article className={styles.article}>
        <header className={styles.header}>
          <h1 className={styles.title}>Neuroprocessing: Пошаговый алгоритм психологической самопомощи</h1>
          <p className={styles.lead}>Полная версия теории с научными обоснованиями, ссылками на исследования и примерами.</p>
        </header>

        {data?.length ? data.map((sec) => <Section key={sec.id} node={sec} />) : <p>Загружаю разделы…</p>}
