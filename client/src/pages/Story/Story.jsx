...
export default function Story() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const snapToken = initialSnap?.rereviewToken ?? null;
        const snapStartedRound = initialSnap?.rereviewStartedRound ?? null;
        ...
        setRemindersOn(story?.remindersEnabled ?? true);
        ...
        if (initialViewY == null) {
          const fromStorages = pickInitialViewYFromStorages(id, initialSnap);
          const fromDb = parseFinite(story?.lastViewContentY);
          setInitialViewY(fromStorages ?? fromDb ?? null);
        }
        ...
        writeSnapshot(id, {
          ...
          rereviewStartedRound: shouldStartRereview
            ? roundToUse
            : (Number.isFinite(snapStartedRound) ? snapStartedRound : null),
        ...
        if (ideaPendingRef.current.size === 0) {
          clearStoryDirty(id);
        }
        ...
        if (doFreeze || startedFromArchive || !canTrustCache) {
  ...
  const ensureIdeaCreated = async (tempId, initial) => {
    ...
    try {
      ...
      const beliefsCopy = (current.beliefs || []).map(b =>
        b.id === tempId
          ? {
              ...b,
              id: created.id,
      ...
      apply({ ...current, beliefs: beliefsCopy });
      return created.id;
    } catch {
      return null;
    }
  ...
  const DEFAULT_PRACTICES = PRACTICES.map(p => ({
    id: p.slug,
    slug: p.slug,
    title: p.title,
  ...
  const showIdeasSpinner = useSmartDelay(ideasLoading, {
    delayIn: isSlow ? 100 : 200,
    minVisible: isSlow ? 400 : 300,
  });
  const showOverlaySmart = useSmartDelay(showOverlay, {
    delayIn: isSlow ? 200 : 300,
    minVisible: isSlow ? 600 : 400,
  });
    ...
    <div key={id}>
      ...
      {!ideasReady ? (
        ...
        <IdeaList
          ...
          practicesById={{ __default: DEFAULT_PRACTICES }}
